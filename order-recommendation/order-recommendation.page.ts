...
export class OrderRecommendationPage extends PageBase {
    ...
    changeVendor(i) {
        ...
        this.pageProvider.save(this.item).then(() => {
            ...
            this.env.showTranslateMessage('erp.app.pages.product.order-recommendation.message.select-vendor-with-value', 'success', i.VendorName);
            this.selectedCount = this.items.filter(d => d.checked).length;
        })
    }
    ...
    async createPO() {
        ...
        if (data && data.IDWarehouse && data.IDStorer) {
            ...
            await loading.present().then(() => {
                ...
                this.commonService.connect('POST', ApiSetting.apiDomain("PURCHASE/Order/CreateFromRecommendation/"), postData).toPromise()
                    .then((resp) => {
                        if (loading) loading.dismiss();
                        this.env.showTranslateMessage('erp.app.pages.product.order-recommendation.message.created-po','success');
                        this.refresh();
                        this.env.publishEvent({ Code: this.pageConfig.pageName });
                    }).catch(err => {
                        console.log(err);
                        this.env.showTranslateMessage('erp.app.pages.product.order-recommendation.message.can-not-create','danger');
                        if (loading) loading.dismiss();
                    })
            })
        }
